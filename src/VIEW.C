/***
* VIEW.C
* Função que exibe todas as figuras desenhadas (todos os elemntos da lista)
...
void view( int nViewMode )
{
    ...
    if( ptrFirst != NULL )
    {
        ...
        setwritemode( XOR_PUT );         // Repõe o modo de escrita do programa
    }

    display_toolbox();                              // Repõe a Toolbox no ecran
    mouse_ShowCursor();                             // Repõe o cursor do mouse
    setcolor( nForeColor );                         // Repõe a cor anterior
    setbkcolor( nBackColor );                  // Repõe a cor de fundo anterior
}
