...
* PAINT.C
* Programa de desenho
* Jorge Morgado, Borland C, CopyRight 1994
*/
...
void main( void )
{
    ...
    initgraph( &gdriver, &gmode, "f:\\borlandc\\bgi" );  // Inicia modo gráfico

    if( graphresult() != grOk )                     // Verifica se ocorreu erro
    {
        printf( "\nErro: inicialização do modo gráfico\n" );
        exit( 1 );
    }

    for( n = 0; n < MAX_COORDS; n++ )                      // Aloca espaço para
        auxCoord[ n ] = ( int * )malloc( sizeof( int ) );  // coords  auxilares

    func_click[ 0 ] = first_click;      // Define as  funções a  ser executadas
    func_click[ 1 ] = second_click;     // quando o botão do  mouse é "clicado"
    ...
    do
    {
        xMouse = mouse_GetXPos();                 // Obtém a posição X do mouse
        yMouse = mouse_GetYPos();                 // Obtém a posição Y do mouse
    ...
    while( ptrFirst != NULL )                   // Elimina  a  lista  existente
        undo();                                 // para libertar toda a memória
